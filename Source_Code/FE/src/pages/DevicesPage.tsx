...
import { Device, DeviceType } from "../interfaces/entities.interface";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {devices.map((device) => (
            <DeviceCard
              key={device.id}
              device={device}
              deviceType={(Array.isArray(deviceTypes) ? deviceTypes : []).find(
...
function DeviceCard({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-shadow relative">
      ...
      {isOn !== null && (
        <div
          className={`absolute top-6 right-6 px-2 py-1 rounded text-xs font-bold border ${
            isOn
